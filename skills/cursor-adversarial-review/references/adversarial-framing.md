<!--
Prompt frame for a cursor-agent review leg, OPTIONAL. The same text as the
codex frame (ask mode can run read-only git, measured 2026-09-24). Equivalence
test 2026-09-24 (three known-answer targets, three runs per arm,
grok-4.7-medium): the plain brief 11, this frame 12, and slower -- inside the
noise, so it is offered, not wired in (SKILL.md says how to use it).
The suite's review-prompt.py substitutes {{BASE}}, {{HEAD}} and {{LENS}}.
-->
