...
| 2026-09-02 | cursor/cursor-grok-4.6-high | review (3 document changes, 7 posed items) | Two sole findings, both real, both the kind only a leg that opens the cited artifact will get: the audit report under review cites an HTTP route that does not exist (the registered route has a different path, one controller header away), and a sibling spec section claims to be "the authoritative summary of current state" on the same reviewer-walkthrough credential the ADR uses for its `Accepted` status — one witness, two documents, no independent check. Both went into the lead's reviews. **But the severity-discount rule from 2026-09-01 held for a second round, and this time the lead had to do real work to refute it.** It argued the audit's severe rating on a resend loop was inflated because a key mismatch means the loop never runs. Measured: the two state maps are keyed by two different id derivations that agree on the standard id family, so the loop is dormant only for the non-standard ids — on the standard path the keys match and it runs. The report already said as much in its own text. **Three rounds, one stable signature: sharpest at finding the artifact nobody opened, and systematically one step too confident when it argues a rating DOWN or calls something a contradiction.** Discount downgrade arguments as hard as contradiction claims. |
| 2026-09-02 | cursor/cursor-grok-4.6-high | review (command-sequencing change, 11 posed items) | **Two sole findings, both load-bearing, and the first round where its dismissals were all correct.** (1) The new confirmed-state sequence checks the device's mode on its first transition and only a second state flag on its second — so after the first step is confirmed, an abort that changes the mode does not stop the final command. That is the exact mirror of the defect the lead had −1'd on the predecessor change (which checked the flag and not the mode); the same hole moved one step down the state machine. (2) It escalated a posed item past what was asked: the frontend command log distinguishes "a step of a sequence" from "the terminal" by *command inequality*, and the sequence's final step carries the same command id as the entry — so the device's acknowledgement of the final step resolves the row as ACCEPTED before the sequence has actually completed, and the in-tree test never injects that acknowledgement. Both lead-verified. **The severity-discount rule did not fire this round**: nine dismissals, all checked, all correct — including calling a service's echo-ordering a non-finding where two other legs (one paid) reported it as a race. Three rounds in, revise the note: discount its *downgrades* only when it is arguing a rating down on someone else's finding; its own HOLDS have been accurate. Still the leg that opens the artifact nobody was told to open. 542 s. |
| 2026-09-04 | cursor/cursor-grok-4.6-high | review (2 planning docs, 9 posed items) | **The only leg of three with a real defect, and it was the round's sole −1 ground.** A fixed-decisions table promised "there is no `NOT_REQUIRED` fallback" for one class of evidence; this leg found that a validator wraps that entire block — including the resolve call — in an `if (… == REQUIRED)` condition, that the migration's CHECK permits `NOT_REQUIRED`, and that **no task in the 710-line plan pins the test-environment policy to REQUIRED**. Lead verified and found it sharper still: the only occurrence of `NOT_REQUIRED` in the whole document is the sentence promising it cannot happen. Also sole on two MAJORs whose premises the host verified — a shared store with no test/production discriminator and a production currentness SELECT with no environment predicate, and an acceptance gate requiring proof of entry into two later phases in a plan whose goal says those phases are not authorized. 518 s, 30 of 31 citations exact. **New failure mode, first observed on this leg: selective quotation.** It quoted a ledger cell as `**Merged**` when the cell reads `**Merged / runtime still disabled**` — line number correct, and the dropped half was precisely the clause that weakened the finding it was supporting. That is distinct from the over-confidence already on file: not a wrong severity, a shortened quote. **Add to the standing note: check its quotes against the full line, not just the line number.** It also shared the round's deadlock over-read with the agy leg, though its own supervisor flagged that as the shape to verify first. |
| 2026-09-04 | cursor/cursor-grok-4.6-high | research (an open-source governance gateway, 4 aspects / 18 lettered questions) | 18/18 answered, zero fabricated citations: **43 of 43 spot-checked citations exact against the frozen tree and a memory-backend clone, including every fenced quote compared FULL-LINE** — the selective-quotation failure mode from 2026-09-04's earlier round did NOT recur. Delivered the brief's stated jackpot: a **sixth** process-local state holder beyond the security document's five (a per-actor rate limiter in an internal credential API, default 10/min, with exact line citations for its declaration, default and enforcement) plus seven further unnamed process-local holders. Also found a single-instance variant of the (a2) credential window the brief did not posit (the key verification runs outside the keystore lock, so one process has the same observable), and a change in the memory backend's search since the 2026-07-12 static reading (a null-session fallback: rows whose session is gone pass through UNSCOPED). **Honest on the trap question**: (a5) returned NOT REACHED for all four backends rather than inventing idempotency-key URLs, while still supplying an in-clone source reading for the memory backend. Discipline held on the research/review distinction — confidence levels, no severities, no recommendations. **New adapter failure mode observed:** one launch produced TWO sessions writing the same stdout file (`f78e939f`/`4dee5e43` complete at 26288 B, `0d3c638d`/`8a578984` overwritten to a 562 B tail) — `--output-format json` output is not guaranteed to be a single object; parse line-wise and expect a possible duplicate dispatch. 509 s. |
...
| 2026-09-05 | cursor/cursor-grok-4.6-**medium** (first `-medium` round) | review (10-change UI stack, 8 posed items, reviewed per-diff) | **The round's best surface reader, at medium.** 330 s, zero NOT REACHED, 57/61 citations exact full-line (4 off-by-one or wrong-file, no selective quotation). Three sole MAJORs, all lead-verified and all in the −1s: (1) a comment says a waiting state counts as group membership, the code requires an explicit membership check, and the test pins the opposite — the exact scenario the comment names is what the code permits; (2) a pending-command confirmation never re-checks the device's current membership, so a command armed while detached fires after the device has rejoined; (3) two different controls with different effects are both labelled "Rejoin". Also the only leg to give the right account of a detached device (it continues to its last target and holds there — no stop is sent) where the gemini leg overclaimed. **Its one downgrade was wrong**: it held a stale placeholder value at MAJOR because the production path fails closed — but the placeholder path is the one in use today; the lead kept BLOCKING. Exhaustiveness: its count of detached ids said 7 and listed 8 (8 reproduces). Diagnostic: with pre-extracted diffs in an external dir the 128 KB argv limit is a non-issue; the JSON came back as one object this time. |
| 2026-09-05 | cursor/cursor-grok-4.6-**medium** | review (ownership-transfer change across four services + frontend, 8 posed items) | **Second `-medium` round; the phase-by-phase table is what this leg is for.** 377 s. Asked what a stop command does to a device that is not in a steady state, it produced a row per execution phase with the command sent, what the status handler then does, and what the new job's start sees — including two consequences no other leg reached: a rejected stop acknowledgement marks the OLD execution `Failed` (the phase's expected command is a different one), and a release during one intermediate phase leads the status handler to clear the job just uploaded to the device. The second converged with big-pickle and became a named MAJOR. It also gave the correct account of a released device where another leg overclaimed. **Pattern across both `-medium` rounds: give it a question whose answer is a table and it enumerates exhaustively; give it a question whose answer is a severity and it under-rates** (it held the previous round's stale-placeholder BLOCKING at MAJOR too). Take its enumerations, set the severities yourself. |
| 2026-09-06 | cursor/cursor-grok-4.6-**medium** | review (a data contract across 3 services + a migration, 10 posed items) | **Third `-medium` round; the enumerate-well / rate-badly split held, but this time it erred UPWARD.** 388 s, 27/28 fenced citations exact full-line, no selective quotation, and 6 of 6 exhaustiveness counts reproduced — the count-vs-list mismatch from its 2026-09-05 row did not recur. Its strongest single output was a table nobody asked for: which of the three hardening rules live in the C++ decoder versus the JSON schema, ending in the finding that **neither service loads the schema at all**, so the schema is documentation and the C++ allowlist is the contract — and the two disagree on one field. Lead-verified. It also independently falsified the commit message's pagination claim by identifying the ORDER BY as a context line. **Its two MAJORs on expiry were both over-rated**: the 'immortal proposal' needs a producer that does not exist in-tree, and its reachability argument named a field the parent revision did write. Its own supervisor caught that and re-rated both to MINOR. **Ruling stands and now has a direction: at `-medium` this leg's severities are unreliable in BOTH directions across rounds — take the enumerations, set every severity yourself.** |
| 2026-09-06 | cursor/cursor-grok-4.6-high | review (falsifiability + surface map, S2 detection) | 8.3 min. Complete reader table for `corporate_actions.status` across backend, shared engine, web and mobile (the web manager was the only path that feeds the VISIBLE list — pending included — into the engine, guarded solely by the engine's own status filter). Named the exact-string de-dup gap (`2330` vs `2330.TW`) that GPT found independently from the loader side — convergence. Three test-hardening items all verified. No overreach this round. |
...
| 2026-09-24 | grok-4.7-medium | review, a peer's four-packet round (C++/SQL/Compose, docs, web package, scripts) | Strongest on the documentation packet: the only real P2 (a wrong owner row), a stale item count and a table-vs-intro mismatch — all three later fixed by the author, 3/3 accepted. **Two false positives from the packet's shape, not the model:** "cited files do not exist" (the frozen tree sat on a parent far behind the target branch, where the files did exist) and "report still draft" (only its front matter was stale) — one became a wrong -1. On code: one finding rebutted by a Compose probe, one confirmed and escalated to an authority bypass. Its votes over-lenient once (+2 with a carried blocker). |
